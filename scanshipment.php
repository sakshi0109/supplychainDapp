...
<html lang="en">
  <head>
    ...
    <title>VITRON - Scan Shipments</title>
  ...
  <body>
        ...
        <div class="mycardstyle">
            <div class="greyarea">
                ...
                <form id="form2" autocomplete="off">
                    <div class="formitem">
                        <label type="text" class="formsensorlabel"> Product ID : </label>
                        <input type="text" class="formsensorinput" id="prodid" onkeypress="isInputNumber(event)" style="width: 200px;" required>
                    ...
                    <div class="formitem">
                        <label type="text" class="formsensorlabel">  Location : </label>
                        <input type="text" class="formsensorinput" id="prodlocation" style="width: 350px;" required><br>
                    </div>
                    <div class="formitem">
                        <label type="text" class="formsensorlabel">  Temperature : </label>
                        <input type="text" class="formsensorinput" id="prodtemp" required>
                        <label type="text" class="formsensorlabel">  Light Intensity : </label>
                        <input type="text" class="formsensorinput" id="prodlight" required>
                    </div>
                    <div class="formitem">
                        <label type="text" class="formsensorlabel">  Humidity : </label>
                        <input type="text" class="formsensorinput" id="prodhumid" required>
                        <label type="text" class="formsensorlabel">  Pressure : </label>
                        <input type="text" class="formsensorinput" id="prodpressure" required>
                    </div>
    ...
    $('#form1').on('submit', function(event) {
        ...
        web3.eth.getAccounts().then(async function(accounts) {
          var receipt = await contract.methods.newItem(prodname, thisdate).send({ from: accounts[0], gas: 1000000 }).then(receipt => {
    ...
    $('#form2').on('submit', function(event) {
        ...
        light = $('#prodlight').val();
        humid = $('#prodhumid').val();
        pressure = $('#prodpressure').val();

        if (temp<=10){
          alert("Temperature should be greater than 10!!!");
        }
        var today = new Date();
        var thisdate = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
        var info = "<br><br><b>Date: "+thisdate+"</b><br>Location: "+prodlocation+"</b><br>Temperature: "+temp+"</b><br>Light Intensity: "+light+"</b><br>Humidity: "+humid+"</b><br>Pressure: "+pressure;
        web3.eth.getAccounts().then(async function(accounts) {
          var receipt = await contract.methods.addState(prodid, info).send({ from: accounts[0], gas: 1000000 }).then(receipt => {
