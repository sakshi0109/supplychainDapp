...
<html lang="en">
  <head>
    ...
    <title>VITRON - Add New Products</title>
  ...
  <body>
              ...
              <p> Please fill product details  </p>
                <form id="form1" autocomplete="off">
                    <div class="formitem">
                        ...
                        <label type="text" class="formlabel"> Product Price </label>
                        <input type="text" class="forminput" id="prodprice" required>
                        <input type="hidden" class="forminput" id="user" value=<?php echo $_SESSION['username']; ?> required><br><br>
                        <h4>Maximum Suitable Conditions : </h4><br>
                        <div id="sensortext">
                          <label type="text" class="formsensorlabel"><strong>Temperature : </strong></label> 
                          <input type="text" class="formsensorinput" id="prodmaxtemp" required>
                          <label type="text" class="formsensorlabel"><strong>Light Intensity : </strong>  </label>
                          <input type="text" class="formsensorinput" id="prodmaxlight" required><br><br>
                          <label type="text" class="formsensorlabel"><strong>Humidity : </strong>  </label>
                          <input type="text" class="formsensorinput" id="prodmaxhumid" required>                          
                          <label type="text" class="formsensorlabel"><strong>Pressure : </strong>  </label>
                          <input type="text" class="formsensorinput" id="prodmaxpressure" required>
                        </div>
    ...
    $('#form1').on('submit', function(event) {
        ...
        username = $('#user').val(); 
        maxtemp = $('#prodmaxtemp').val(); 
        maxlight = $('#prodmaxlight').val(); 
        maxhumid = $('#prodmaxhumid').val(); 
        maxpressure = $('#prodmaxpressure').val(); 
        prodname=prodname+"<br>Product Price: "+prodprice+"<br>Registered By: "+username+"<br>Max. Temperature: "+maxtemp+"<br>Max. Light Intensity: "+maxtemp+"<br>Max. Humidity: "+maxhumid+"<br>Max. Pressure: "+maxpressure;
        console.log(prodname);
        var today = new Date();
        var thisdate = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();

        web3.eth.getAccounts().then(async function(accounts) {
          var receipt = await contract.methods.newItem(prodname, thisdate, prodprice, maxtemp, maxhumid, maxlight, maxpressure).send({ from: accounts[0], gas: 1000000 }).then(receipt => {
